...
export class Player extends Actor {
    ...
    public playerAttackValue: number;

    constructor(_playerName: string, _playerAttackValue: number, _health: number, _maxHealth: number, _position: Room, _inventory?: Item[]) {
    ...
    playerMovement(_playerDirectionInput: string) {
        ...
        switch (_playerDirectionInput) {
            ...
            case "n": {
                ...
                else document.getElementById("combatText").innerHTML = " There is no path in that direction."
                break;
            }
            case "s": {
    ...
    playerLookAt(_wantedObject: string) {
    ...
    attackTarget(_targetString: string) {

        let targetNPC: NPC;
        let combatText: string = "";
        ...
        if (targetNPC != null) {
            let prescision: number = Math.floor(Math.random() * 5);
            switch (prescision) {
                case 0:
                case 1:
                case 2:
    ...
    talkToTarget(_targetString: string) {
        let targetNPC: NPC;
