...
function getMenuUserInput(_userInput) {
    let userInputArray = _userInput.split(" ");
    let userActionInput = userInputArray[0];
    let userObjectInput = userInputArray[1];
...
function createPlayerCharacter(_userName) {
...
function getPlayerInput(_userInput) {
    let userInputArray = _userInput.split(" ");
    let userActionInput = userInputArray[0];
    let userObjectInput = userInputArray[1];
    ...
    switch (userActionInput) {
        ...
        case "load": {
            let loadedSaveFileAny = JSON.parse(localStorage.getItem(currentPlayer.playerName));
...
function handleMenuInputEvent() {
    getMenuUserInput(document.getElementById("menuUserInput").value);
    menuUserInput.value = "";
}
function handlePlayerNameInputEvent() {
    createPlayerCharacter(document.getElementById("menuUserInput").value);
    menuUserInput.value = "";
}
function handlePlayerInputEvent() {
    getPlayerInput(document.getElementById("menuUserInput").value);
    menuUserInput.value = "";
}
function handleLoadingMenu(_userObjectInput) {
    if (_userObjectInput == undefined || _userObjectInput == "" || _userObjectInput == null) {
        ...
        let loadedSaveFileAny = JSON.parse(localStorage.getItem(_userObjectInput));
        let loadedSaveFile = new SaveFile();
        loadedSaveFile.roomKeysWithContentKeys = loadedSaveFileAny.roomKeysWithContentKeys;
        loadedSaveFile.npcKeysWithInformation = loadedSaveFileAny.npcKeysWithInformation;
        loadedSaveFile.playerHealth = loadedSaveFileAny.playerHealth;
...
function handleNpcAI(_player) {
    for (let i = 0; i < map.length; i++) {
        for (let j = 0; j < map[i].roomCharacters.length; j++) {
